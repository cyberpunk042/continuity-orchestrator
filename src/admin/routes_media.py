...
GitHub Release vault routes (upload/download/cancel for large files) are in
routes_media_vault.py.
"""
...
def api_reoptimize():
    ...
    from .routes_media_vault import upload_to_release_bg
    for item in results["optimized"]:
        if item["new_tier"] == "large":
            upload_to_release_bg(item["id"], manifest.enc_path(item["id"]))
...
def api_upload_media():
    ...
    try:
        ...
        if storage_tier == "large":
            from .routes_media_vault import upload_to_release_bg
            upload_to_release_bg(media_id, enc_path)
...
def api_delete_media(media_id: str):
    ...
    if storage == "large":
        from .routes_media_vault import delete_release_asset
        delete_release_asset(media_id)
